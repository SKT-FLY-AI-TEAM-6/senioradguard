...
/**
 * "광고를 눌러 다른 사이트에 닿았는가"를 **주소가 바뀐 흔적**으로 판단한다.
 * (phase6 RedirectChainTracker 이식 — 병합 계획의 이름 RelayTransitTracker로 들여온다)
 *
 * ## 왜 클릭 이벤트로는 안 되는가
 * 크롬은 웹 광고를 눌러도 TYPE_VIEW_CLICKED를 보내지 않는다(phase6 실기기 확인).
 * 웹 콘텐츠는 가상 노드 트리라 위젯 클릭으로 취급되지 않기 때문이다. phase4의
 * AdEntryDetector가 클릭 좌표·리다이렉터 URL로 대부분을 잡지만, 주소창이 접혀
 * URL을 못 읽는 순간의 중계 경유는 이쪽 흔적만 남는다.
 *
 * ## 무엇을 보는가
 ...
 *
 * ## 이 판정이 틀리는 경우
 * 광고가 중계 없이 곧바로 목적지로 보내면 놓친다(반응을 안 함). 반대로 사용자가
 * 검색 결과처럼 중계성 링크를 거쳐 들어가면 잘못 반응할 수 있다. 놓치는 쪽이
 * 덜 해로우므로 증거가 없으면 조용히 있는 쪽을 택한다.
 */
class RelayTransitTracker(private val now: () -> Long = System::currentTimeMillis) {
