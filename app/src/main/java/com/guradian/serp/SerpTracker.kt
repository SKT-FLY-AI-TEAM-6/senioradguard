...
class SerpTracker(
    ...
    companion object {
         ...
         *
         * 700ms였는데 실사용에서 "배지가 늦게 뜬다"는 리포트가 나왔다. 이 대기는
         * 판정 **시작**을 미루는 값이라 판별기 왕복 시간에 그대로 얹힌다 — 첫 배지가
         * 뜨기까지 체감이 그만큼 길어진다. 크롬의 스크롤 이벤트 간격(약 100ms)보다
         * 넉넉히 길기만 하면 "스크롤이 멈췄다"는 판단에는 충분하다.
         */
    ...
    /**
     * 지금 화면이 검색 결과 화면인가. 서비스가 도메인 대조(UrlGuard)를 건너뛸지
     * 정할 때 읽는다 — 검색 결과 위에서는 이 기능의 배지가 이미 같은 위험을
     * 더 정밀하게(결과 칸 단위로) 알리고 있다.
     */
    @Volatile
    var isSerpScreen: Boolean = false
        private set

    /** [onHighRisk]를 이미 부른 호스트들. 같은 호스트로 두 번 승격하지 않는다. */
    private val promotedHosts = mutableSetOf<String>()
    ...
    private suspend fun runScan(judge: Boolean) {
        try {
            ...
            // 최고 위험 등급만 원격 승격 — 새로 확인된 호스트당 한 번.
            // 호스트 자체는 기기 밖으로 나가지 않는다 (promotedHosts는 로컬 기억).
            val newlyHigh = outcome.results.zip(outcome.verdicts)
                .filter { (_, v) -> v.grade == RiskGrade.HIGH }
                .map { (r, _) -> r.host }
