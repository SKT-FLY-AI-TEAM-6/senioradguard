...
class GeminiClassifier(
    ...
    companion object {
         ...
         * 다시 볼 기회가 사라진다) 상한에 걸린 상태에서는 누를 때마다 같은 카드를 다시
         * 시도하게 된다. 실기기에서 몇 초 만에 호출 수십 건이 나가는 것을 확인해 넣었다.
         */
        private const val QUOTA_COOLDOWN_MS = 5 * 60 * 1000L

        /**
         * 사용자가 [광고 찾기]를 누르고 기다리는 동안 도는 호출이다. 무한정 매달리면
         * "찾는 중…"이 영영 끝나지 않는다. 실측 응답이 2초대라 읽기 8초면 정상 범위를
         * 넉넉히 덮으면서 사람이 포기하기 전에 끝난다.
         */
        ...
        try {
            ...
            if (conn.responseCode != HttpURLConnection.HTTP_OK) {
                ...
                if (conn.responseCode == 429) {
                    ...
                    // 그동안 Layer 1과 캐시는 그대로 동작한다 — 조용히 얕아질 뿐이다.
