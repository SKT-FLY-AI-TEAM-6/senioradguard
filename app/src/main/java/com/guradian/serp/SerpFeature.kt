...
class SerpFeature(
    ...
    apiKey: String = "",
    /**
     * 최고 위험([RiskGrade.HIGH]) 판정이 새 호스트에서 나올 때마다 한 번 부른다.
     * 서비스가 원격 승격(SEARCH_RISK_DETECTED)에 쓴다. 호스트는 넘기지 않는다.
     */
    onHighRisk: () -> Unit = {}
) {
    ...
    private val tracker = SerpTracker(scanner, engine, overlay, scope, onHighRisk)

    /**
     * 지금 화면이 검색 결과 화면인가. 서비스가 도메인 대조(UrlGuard)를 건너뛸 때
     * 읽는다 — 검색 결과 위에서는 배지가 결과 칸 단위로 이미 같은 위험을 알린다.
     */
    ...
    /**
     * 배지만 잠시 걷는다 (수신기·판정 캐시는 유지). 가림막·설치 경고 같은
     * HIGH 위험 오버레이가 떠 있는 동안 서비스가 부른다 — 화면을 덮은 경고
     * 위에 배지까지 겹치면 어느 것도 읽히지 않는다. 경고가 걷히고 이벤트가
     * 다시 들어오면 다음 스캔이 배지를 되살린다.
     */
