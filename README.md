# GuArDian (구 SeniorAdGuard)

> **⚠️ 방향 전환 (2026-08-14, 브랜치 `phase4-guardian-pivot`)**
> 피드백을 반영해 프로젝트를 "광고 표시 편의 기능"에서 **광고에서 시작되는
> 디지털 피해 예방 서비스**로 재정의했다. 광고를 일괄 제거하지 않고, 위험도
> (저/중/고 + 추가 확인 필요)를 판단해 표시·확인·차단으로 대응한다. 외부 LLM
> API 대신 온디바이스 분석으로 전환한다. 기존 3-Layer 코드는 새 아키텍처의
> 부품으로 대부분 유지된다.
> 이행 계획: `docs/superpowers/specs/2026-08-14-guardian-pivot-design.md`
> 아래 문서는 피벗 이전 기준이며, 동작 설명은 여전히 유효하다.
...
| **Layer 2** | 라벨 없는 광고를 LLM으로 추정 | 노랑 **점선** + "광고의심" | 통과 |
...
점선이 틀릴 수 있다는 뜻을 UI로 구분해둔 것이다 — 실선/점선, "광고"/"광고의심". 추정 쪽에는 알약을 달지 않는다. "AI"는 어르신에게 아무것도 설명하지 못하면서 두 배지를 닮은 모양으로 만들 뿐이다.
