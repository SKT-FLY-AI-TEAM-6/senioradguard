...
/**
 * 광고를 눌렀을 때 흔히 끌려가는 곳의 **이름표**. DB를 쓰지 않고 상수로만 관리한다.
 *
 * 예전에는 이 목록이 "경고할지 말지"를 정했다. 지금은 아니다 — 광고를 거쳐 왔다는
 * 사실 자체로 경고하고, 이 목록은 `com.einnovation.temu`를 "테무"로 바꿔 보여주는
 * 데만 쓴다. 목록에 없으면 도메인을 그대로 보여준다.
 *
